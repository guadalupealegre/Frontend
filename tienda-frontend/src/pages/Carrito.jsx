...
export default function Carrito() {
  ...
  const confirmar = async () => {
    // 1. Prevención estricta de doble submit
    if (enviando) return;
    ...
    try {
      // 3. Llamada al servicio con el payload limpio {"items": [{"producto_id": X, "cantidad": Y}]}
      const pedidoCreado = await crearPedido(items, token);

      // 4. Éxito: vaciar carrito y notificar
      vaciar();
      agregarToast(`¡Pedido #${pedidoCreado.id} confirmado con éxito! 🎉`, 'success');

      // 5. Redirección a la vista de Mis Pedidos
      navigate('/mis-pedidos', {
        state: { nuevoPedidoId: pedidoCreado.id },
      });
    } catch (err) {
      // Manejo de errores de stock (409 Conflict) y otros
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-3xl mx-auto py-12 px-4 text-center space-y-6 animate-fade-in">
        <div className="bg-white rounded-3xl p-10 sm:p-16 border border-amber-100 shadow-warm space-y-6">
          <div className="w-24 h-24 mx-auto rounded-3xl bg-amber-50 border-2 border-amber-200/80 flex items-center justify-center text-5xl shadow-inner">
            🧁
          </div>
          ...
          <div className="pt-2">
            <Link
              to="/"
              className="inline-flex items-center space-x-2 px-6 py-3.5 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold text-sm shadow-md shadow-orange-500/25 transition-all active:scale-95"
            >
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-amber-200/60 pb-5">
        <div>
          <h1 className="font-display font-extrabold text-2xl sm:text-3xl text-stone-900 flex items-center space-x-3">
            ...
            <span className="text-sm font-semibold px-3 py-1 rounded-full bg-orange-100 text-orange-800 border border-orange-200">
      ...
      {errorStock && (
        <div className="bg-rose-50 border-2 border-rose-200 text-rose-900 p-5 rounded-3xl flex items-start space-x-3.5 shadow-sm animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="bg-white rounded-3xl border border-amber-100 shadow-sm p-4 sm:p-6 space-y-4">
            ...
            {items.map(({ producto, cantidad }) => {
              ...
              return (
                <div
                  key={producto.id}
                  className="p-4 sm:p-3 rounded-2xl bg-amber-50/40 border border-amber-100/80 flex flex-col sm:grid sm:grid-cols-12 items-center gap-3 transition-all hover:bg-amber-50/70"
                >
                  {/* Info Producto */}
                  <div className="sm:col-span-6 flex items-center space-x-3.5 w-full">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-100 to-orange-100 border border-amber-200 flex items-center justify-center text-3xl shrink-0 shadow-xs">
                      {emoji}
                    </div>
                  ...
                  <div className="sm:col-span-2 flex items-center justify-center space-x-1.5 w-full sm:w-auto justify-between sm:justify-center">
                    ...
                    <div className="flex items-center space-x-1 bg-white rounded-xl p-1 border border-stone-200 shadow-xs">
                      <button
                        ...
                        className="w-7 h-7 rounded-lg bg-stone-50 hover:bg-stone-100 text-stone-700 hover:text-orange-600 flex items-center justify-center transition-colors"
                      ...
                      <button
                        ...
                        className="w-7 h-7 rounded-lg bg-stone-50 hover:bg-stone-100 text-stone-700 hover:text-orange-600 flex items-center justify-center transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  ...
                  <div className="sm:col-span-2 flex items-center justify-between sm:justify-end space-x-3 w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-0 border-amber-100">
          ...
          <div className="flex items-center justify-between pt-2">
            <Link
              to="/"
              className="inline-flex items-center space-x-2 text-xs font-bold text-orange-700 hover:text-orange-900 transition-colors"
            >
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-3xl border border-amber-100 shadow-sm p-6 space-y-5">
            ...
            <div className="space-y-2.5 text-xs sm:text-sm">
              ...
              <div className="pt-3 border-t border-stone-100 flex items-baseline justify-between">
                ...
                <span className="font-display font-extrabold text-2xl sm:text-3xl text-amber-950 tracking-tight">
            ...
            <div className="p-3.5 rounded-2xl bg-amber-50/70 border border-amber-200/60 space-y-1.5 text-xs text-amber-900">
            ...
            <button
              ...
              className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-orange-500 via-orange-600 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold text-base shadow-lg shadow-orange-500/25 flex items-center justify-center space-x-2 transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none"
            >
              {enviando ? (
                <>
